	...
	any function html( required struct meta,
		...
		var indent = repeatString(variables.TAB, arguments.level);
			...
			if(arguments.level EQ 0){
				...
				head&='titleTD.onclick=function(){ currDumpObj.META.ONCLICK };' & variables.NEWLINE;
				...
				head&='tbdyTRs[j].name=currDumpObj.META.DATA[j].NODEID;' & variables.NEWLINE;
				head&='if(currDumpObj.META.DATA[j].HIDDEN)' & variables.NEWLINE;
