...
public class RHExtension implements Serializable {
	...
	public RHExtension(Config config, Resource ext, boolean moveIfNecessary)
			throws PageException, IOException, BundleException {
		this.config = config;
	...
	public static Manifest getManifestFromFile(Config config, Resource file)
	...
	private void load(Resource ext) throws IOException, BundleException, ApplicationException {
			...
			while ((entry = zis.getNextEntry()) != null) {
				...
				// jars
						...
						&& (StringUtil.endsWithIgnoreCase(path, ".jar")
								|| (isPack200 = StringUtil.endsWithIgnoreCase(path, ".jar.pack.gz")))) {
				...
				// flds
						...
						&& (StringUtil.endsWithIgnoreCase(path, ".fld")
								|| StringUtil.endsWithIgnoreCase(path, ".fldx")))
					flds.add(fileName);

				// tlds
				else if (!entry.isDirectory() && startsWith(path, type, "tlds")
						&& (StringUtil.endsWithIgnoreCase(path, ".tld")
								|| StringUtil.endsWithIgnoreCase(path, ".tldx")))
					tlds.add(fileName);

				// archives
				else if (!entry.isDirectory()
						&& (startsWith(path, type, "archives") || startsWith(path, type, "mappings"))
						&& StringUtil.endsWithIgnoreCase(path, ".lar"))
				...
				// context
				else if (!entry.isDirectory() && startsWith(path, type, "context")
						&& !StringUtil.startsWith(fileName(entry), '.'))
					contexts.add(sub);

				// web contextS
				else if (!entry.isDirectory()
						&& (startsWith(path, type, "webcontexts") || startsWith(path, type, "web.contexts"))
						&& !StringUtil.startsWith(fileName(entry), '.'))
					webContexts.add(sub);

				// config
				else if (!entry.isDirectory() && startsWith(path, type, "config")
						&& !StringUtil.startsWith(fileName(entry), '.'))
					configs.add(sub);

				// applications
				else if (!entry.isDirectory() && (startsWith(path, type, "web.applications")
						|| startsWith(path, type, "applications") || startsWith(path, type, "web"))
						&& !StringUtil.startsWith(fileName(entry), '.'))
					applications.add(sub);

				// components
				else if (!entry.isDirectory() && (startsWith(path, type, "components"))
						&& !StringUtil.startsWith(fileName(entry), '.'))
					components.add(sub);

				// plugins
				else if (!entry.isDirectory() && (startsWith(path, type, "plugins"))
						&& !StringUtil.startsWith(fileName(entry), '.'))
					plugins.add(sub);
		...
		if (manifest == null)
			throw new ApplicationException(
					"The Extension [" + ext + "] is invalid,no Manifest file was found at [META-INF/MANIFEST.MF].");
	...
	private void readMapping(String label, String str, Log logger) {
		...
		if (mappings == null)
			mappings = new ArrayList<Map<String, String>>();
	}
	...
	private void readEventGatewayInstances(String label, String str, Log logger) {
		...
		if (eventGatewayInstances == null)
			eventGatewayInstances = new ArrayList<Map<String, Object>>();
	}
	...
	private void readJDBC(String label, String str, Log logger) {
		...
		if (jdbcs == null)
			jdbcs = new ArrayList<Map<String, String>>();
	}
	...
	private void readCacheHandler(String label, String str, Log logger) {
		...
		if (cacheHandlers == null)
			cacheHandlers = new ArrayList<Map<String, String>>();
	}
	...
	private void readCache(String label, String str, Log logger) {
		...
		if (caches == null)
			caches = new ArrayList<Map<String, String>>();
	}
	...
	private void readMonitor(String label, String str, Log logger) {
		...
		if (monitors == null)
			monitors = new ArrayList<Map<String, String>>();
	}
	...
	private void readSearch(String label, String str, Log logger) {
		...
		if (searchs == null)
			searchs = new ArrayList<Map<String, String>>();
	}
	...
	private void readResource(String label, String str, Log logger) {
		...
		if (resources == null)
			resources = new ArrayList<Map<String, String>>();
	...
	private void readAMF(String label, String str, Log logger) {
		...
		if (amfs == null)
			amfs = new ArrayList<Map<String, String>>();
	}
	...
	private void readLoaderVersion(String label, String str) throws ApplicationException {
		...
		if (minLoaderVersion > SystemUtil.getLoaderVersion()) {
			throw new InvalidVersion("The Extension [" + label + "] cannot be loaded, " + Constants.NAME
	...
	private void readCoreVersion(String label, String str, Info info) throws ApplicationException {
		...
		if (minCoreVersion != null && Util.isNewerThan(minCoreVersion, info.getVersion())) {
			// PATCH for MSSQL
			if ("99A4EF8D-F2FD-40C8-8FB8C2E67A4EEEB6".equals(id)) {
				Version min = OSGiUtil.toVersion("5.2.9.39-SNAPSHOT", null);
				Version max = OSGiUtil.toVersion("5.3.0.0-SNAPSHOT", null);
				// bigger than 5.2.9.39-SNAPSHOT
				if (Util.isNewerThan(info.getVersion(), min)) {
					// smaller than 5.3.0.0-SNAPSHOT
					if (Util.isNewerThan(max, info.getVersion())) {
						return; // we are fine!
					}
				}
			}
			...
			throw new InvalidVersion(
					"The Extension [" + label + "] cannot be loaded, " + Constants.NAME + " Version must be at least ["
	...
	private void readReleaseType(String label, String str, boolean isWeb) throws ApplicationException {
		...
		if (!Util.isEmpty(str)) {
			...
			if ("server".equalsIgnoreCase(str))
				rt = RELEASE_TYPE_SERVER;
			else if ("web".equalsIgnoreCase(str))
				rt = RELEASE_TYPE_WEB;
		}
		if ((rt == RELEASE_TYPE_SERVER && isWeb) || (rt == RELEASE_TYPE_WEB && !isWeb)) {
			throw new ApplicationException("Cannot install the Extension [" + label + "] in the " + type
					+ " context, this Extension has the release type [" + toReleaseType(rt, "") + "].");
		}
		releaseType = rt;
	}
	...
	private void readId(String label, String id) throws ApplicationException {
		...
		if (!Decision.isUUId(id)) {
			throw new ApplicationException(
					"The Extension [" + label + "] has no valid id defined (" + id + "),id must be a valid UUID.");
		}
	}
	...
	private void readVersion(String label, String version) throws ApplicationException {
		...
		if (StringUtil.isEmpty(version)) {
			throw new ApplicationException(
					"cannot deploy extension [" + label + "], this Extension has no version information.");
		}
		...
		if (!ArrayUtil.isEmpty(arrStrs)) {
			...
			for (int i = 0; i < arrStrs.length; i++) {
				...
				if (index == -1)
					...
					try {
						arrBDs[i] = new BundleDefinition(arrStrs[i].substring(0, index).trim(),
								arrStrs[i].substring(index + 1).trim());
	...
	private static void pop(Element el, Attributes attr, String name, String defaultValue) {
		...
		if (!StringUtil.isEmpty(val))
			...
			el.setAttribute(name, defaultValue);
		else
	...
	public void populate(Element el) {
		...
		// newly added
		// start bundles (IMPORTANT:this key is used to reconize a newer entry, so do
		// not change)
		...
		if (!StringUtil.isEmpty(eventGatewayInstancesJson))
			...
			el.removeAttribute("event-gateway-instances");
	}
	...
	private static Query createQuery() throws DatabaseException {
		return new QueryImpl(new Key[] { KeyConstants._id, KeyConstants._version, KeyConstants._name,
				KeyConstants._description, KeyConstants._image, RELEASE_TYPE, TRIAL, CATEGORIES, START_BUNDLES, BUNDLES,
				FLDS, TLDS, TAGS, FUNCTIONS, CONTEXTS, WEBCONTEXTS, CONFIG, APPLICATIONS, COMPONENTS, PLUGINS,
				EVENT_GATEWAYS, ARCHIVES }, 0, "Extensions");
	}
	...
	private void populate(Query qry) throws PageException, IOException, BundleException {
		...
		Query qryBundles = new QueryImpl(new Key[] { KeyConstants._name, KeyConstants._version }, bfs.length,
				"bundles");
	...
	public static ClassDefinition<?> toClassDefinition(Config config, Map<String, ?> map,
			ClassDefinition<?> defaultValue) {
		...
		if (StringUtil.isEmpty(_name))
			_name = Caster.toString(map.get("bundleName"), null);
		if (StringUtil.isEmpty(_name))
			_name = Caster.toString(map.get("bundlename"), null);
		if (StringUtil.isEmpty(_name))
			_name = Caster.toString(map.get("name"), null);

		String _version = Caster.toString(map.get("bundle-version"), null);
		if (StringUtil.isEmpty(_version))
			_version = Caster.toString(map.get("bundleVersion"), null);
		if (StringUtil.isEmpty(_version))
			_version = Caster.toString(map.get("bundleversion"), null);
		if (StringUtil.isEmpty(_version))
			_version = Caster.toString(map.get("version"), null);
		if (StringUtil.isEmpty(_class))
			return defaultValue;
	...
	private static boolean startsWith(String path, String type, String name) {
		return StringUtil.startsWithIgnoreCase(path, name + "/")
				|| StringUtil.startsWithIgnoreCase(path, type + "/" + name + "/");
	}
	...
	private static BundleDefinition toBundleDefinition(InputStream is, String name, String extensionVersion,
	...
	public static String toReleaseType(int releaseType, String defaultValue) {
		if (releaseType == RELEASE_TYPE_WEB)
			return "web";
		if (releaseType == RELEASE_TYPE_SERVER)
			return "server";
		if (releaseType == RELEASE_TYPE_ALL)
			return "all";
		return defaultValue;
	}

	public static int toReleaseType(String releaseType, int defaultValue) {
		if ("web".equalsIgnoreCase(releaseType))
			return RELEASE_TYPE_WEB;
		if ("server".equalsIgnoreCase(releaseType))
			return RELEASE_TYPE_SERVER;
		if ("all".equalsIgnoreCase(releaseType))
			return RELEASE_TYPE_ALL;
		if ("both".equalsIgnoreCase(releaseType))
			return RELEASE_TYPE_ALL;
		return defaultValue;
	}
