...
public final class FormImpl extends ScopeSupport implements Form, ScriptProtected {
	...
	public void initialize(PageContext pc) {
		if(encoding == null)
			encoding = pc.getWebCharset().name();

		if(scriptProtected == ScriptProtected.UNDEFINED) {
			scriptProtected = ((pc.getApplicationContext().getScriptProtect() & ApplicationContext.SCRIPT_PROTECT_FORM) > 0) ? ScriptProtected.YES
					: ScriptProtected.NO;
		}
		super.initialize(pc);
	...
	private void initializeMultiPart(PageContext pc, boolean scriptProteced) {
		...
		FileItemFactory factory = tempDir instanceof File ? new DiskFileItemFactory(DiskFileItemFactory.DEFAULT_SIZE_THRESHOLD, (File)tempDir)
	...
	public FormItem[] getFileItems() {
		if(_fileItems == null || _fileItems.isEmpty())
			return new FormImpl.Item[0];
		Iterator<Item> it = _fileItems.values().iterator();
	...
	public FormItem getUploadResource(String key) {
		...
		while(it.hasNext()) {
			entry = it.next();
			item = entry.getValue();
			if(item.getFieldName().equalsIgnoreCase(keyC))
				return item;

			// /file.tmp
			if(item.getResource().getAbsolutePath().equalsIgnoreCase(key))
				return item;
