				...
				if (hasResult) {
					...
					if (fillResult(qry, qr, keyName, dc, stat.getResultSet(), maxrow, true, createGeneratedKeys, tz)) {
						...
						if( resultsetCount == 1 && !hasPossibleGeneratedKeys ){
							...
							try {
								/*
								 * The Microsoft SQL Server driver can advance the resultset when the getGeneratedKeys()
								 * method is called. If the next resultset happens to have an exception, we have to be
								 * able to track that exception so we can report it. 
								 */
								...
								*/
								hasPossibleGeneratedKeys = !qry.setGeneratedKeys(dc, rs, tz);
								 ...
								 * 
								 * However, we should report other exceptions because they could becoming from a RAISERROR or
								 * other exception being generated by SQL Server.
								 */
								if( DataSourceUtil.isMSSQLDriver(dc) && se.getMessage() != "The statement must be executed before any results can be obtained." ){
									throw se;
								} else {
