...
public final class IsInstanceOf implements Function {
	public static boolean call(PageContext pc, Object obj, String typeName) throws PageException {
		...
		if (obj instanceof JavaObject) {
			try {
				return Reflector.isInstaneOf(((PageContextImpl) pc).getRPCClassLoader(null), ((JavaObject) obj).getClazz(), typeName);
			}
		...
		try {
			return Reflector.isInstaneOf(((PageContextImpl) pc).getRPCClassLoader(null), obj.getClass(), typeName);
		}
		catch (IOException ioe) {
			throw Caster.toPageException(ioe);
		}
