...
public final class QueryMap extends BIF {
	...
	private static Query _call(PageContext pc, Query qry, UDF udf, Query resQuery, String parallel, int maxThreads) throws PageException {
	...
	public Object invoke(PageContext pc, Object[] args) throws PageException {
		...
		if (args.length == 4) return call(pc, Caster.toQuery(args[0]), Caster.toFunction(args[1]), Caster.toQuery(args[2]), Caster.toString(args[3]));
		if (args.length == 5)
			return call(pc, Caster.toQuery(args[0]), Caster.toFunction(args[1]), Caster.toQuery(args[2]), Caster.toString(args[3]), Caster.toDoubleValue(args[4]));
