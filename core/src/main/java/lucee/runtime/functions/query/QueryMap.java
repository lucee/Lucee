...
public class QueryMap extends BIF {
	...
	public Object invoke(PageContext pc, Object[] args) throws PageException {
		...
		if(args.length==3)
			return call(pc, Caster.toQuery(args[0]), Caster.toFunction(args[1]), Caster.toQuery(args[2]));
		if(args.length==4)
			return call(pc, Caster.toQuery(args[0]), Caster.toFunction(args[1]), Caster.toQuery(args[2]), Caster.toBooleanValue(args[3]));
		if(args.length==5)
