...
public final class JavaProxy implements Function {
	...
	public static Class<?> loadClass(PageContext pc , String className, Object pathOrName, String delimiterOrVersion) throws PageException {
		...
		if (StringUtil.isEmpty(pathOrName))
			return loadClassByPath(pc, className, null);
		...
		if (str != null) {
			...
			// path
			if (StringUtil.isEmpty(delimiterOrVersion))
				delimiterOrVersion = ",";

	        String[] arrPaths = ListUtil.trimItems(
	        		ListUtil.toStringArray(
	        				ListUtil.listToArrayRemoveEmpty(str, delimiterOrVersion)
					)
			);
	...
	private static Class<?> loadClassByPath(PageContext pc , String className, String[] paths) throws PageException {
    	...
    	try	{
    		...
    		ClassLoader cl = resources.isEmpty() ?
					pci.getClassLoader()
						:
