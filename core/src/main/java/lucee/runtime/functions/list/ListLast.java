...
public final class ListLast extends BIF {
	...
	if (args.length == 1) return ListUtil.last(Caster.toString(args[0]), ",", false);
	if (args.length == 2) return ListUtil.last(Caster.toString(args[0]), Caster.toString(args[1]), false);
