...
public final class FindOneOf extends BIF {
	...
	public static Number call(PageContext pc, String set, String str) {
		return call(pc, set, str, 1);
	}

	public static Number call(PageContext pc, String strSet, String strData, Number number) {
		// strData
		char[] data = strData.toCharArray();
		// set
		char[] set = strSet.toCharArray();
		// start
		int start = Caster.toIntValue(number) - 1;
		if (start < 0) start = 0;

		if (start >= data.length || set.length == 0) return Caster.toNumber(pc, 0);
		// else {
		for (int i = start; i < data.length; i++) {
			for (int y = 0; y < set.length; y++) {
				if (data[i] == set[y]) return Caster.toNumber(pc, i + 1);
			}
		}
		// }
