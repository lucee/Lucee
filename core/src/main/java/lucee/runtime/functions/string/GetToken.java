...
public final class GetToken extends BIF {
	...
	public static String call(PageContext pc, String str, Number index) throws ExpressionException {
		return call(pc, str, index, null);
	}

	public static String call(PageContext pc, String str, Number index, String delimiters) throws ExpressionException {
		if (delimiters == null) delimiters = "\r\n\t ";
		int idx = Caster.toIntValue(index);
		if (idx < 1) throw new FunctionException(pc, "getToken", 2, "index", "index must be a positive number now (" + idx + ")");

		StringTokenizer tokens = new StringTokenizer(str, delimiters);
		int count = 0;
		while (tokens.hasMoreTokens()) {
			if (++count == idx) return tokens.nextToken();
			tokens.nextToken();
		}
		return "";
	}
