...
/**
 * page context for every page object. the PageContext is a jsp page context expanded by CFML functionality. for example you have the method getSession to get
 * jsp combatible session object (HTTPSession) and with sessionScope() you get CFML combatible session object (Struct,Scope).
 */
public final class PageContextImpl extends PageContext {
	 ...
	 * @param scopeContext
	 * @param config
	 *            Configuration of the CFML Container
	 * @param queryCache
	 *            Query Cache Object
	 * @param id
	...
	/*
	 * public void setRequestTimeoutException(Throwable requestTimeoutException) { this.requestTimeoutException=requestTimeoutException;
	 * 
	 * }
	 */
	...
	@Override
	public void initialize(Servlet servlet, ServletRequest req, ServletResponse rsp, String errorPageURL, boolean needsSession, int bufferSize,
	...
	public void release() {
		...
		if(config.debug()) {
			...
			debugger.resetTraces(); // traces can alo be used when debugging is off
	...
	public PageSource getRelativePageSource(String realPath) {
		...
		if(StringUtil.startsWith(realPath, '/'))
			return PageSourceImpl.best(getPageSources(realPath));
		if(pathList.size() == 0)
			return null;
	...
	public PageSource getRelativePageSourceExisting(String realPath) {
		if(StringUtil.startsWith(realPath, '/'))
			return getPageSourceExisting(realPath);
		if(pathList.size() == 0)
			return null;
		PageSource ps = pathList.getLast().getRealPage(realPath);
		if(PageSourceImpl.pageExist(ps))
			return ps;
		return null;
	}
	 ...
	 * @param realPath
	 * @param previous
	 *            relative not to the caller, relative to the callers caller
	 * @return
	 */
	...
	public PageSource[] getRelativePageSources(String realPath) {
		if(StringUtil.startsWith(realPath, '/'))
			return getPageSources(realPath);
		if(pathList.size() == 0)
			return null;
	...
	public void doInclude(String realPath, boolean runOnce, Object cachedWithin) throws PageException {
		if(cachedWithin == null)
			cachedWithin = getCachedWithin(ConfigWeb.CACHEDWITHIN_INCLUDE);
	...
	public Scope scope(String strScope, Scope defaultValue) throws PageException {
		if(strScope == null)
			return defaultValue;
		if(ignoreScopes()) {
			if("arguments".equals(strScope))
				return argumentsScope();
			if("local".equals(strScope))
				return localScope();
			if("request".equals(strScope))
				return requestScope();
			if("variables".equals(strScope))
				return variablesScope();
			if("server".equals(strScope))
				return serverScope();
			return defaultValue;
		}
		...
		if("variables".equals(strScope))
			return variablesScope();
		if("url".equals(strScope))
			return urlScope();
		if("form".equals(strScope))
			return formScope();
		if("request".equals(strScope))
			return requestScope();
		if("cgi".equals(strScope))
			return cgiScope();
		if("application".equals(strScope))
			return applicationScope();
		if("arguments".equals(strScope))
			return argumentsScope();
		if("session".equals(strScope))
			return sessionScope();
		if("server".equals(strScope))
			return serverScope();
		if("cookie".equals(strScope))
			return cookieScope();
		if("client".equals(strScope))
			return clientScope();
		if("local".equals(strScope))
			return localScope();
		if("cluster".equals(strScope))
			return clusterScope();
	...
	public URLForm urlFormScope() {
		if(!urlForm.isInitalized())
			urlForm.initialize(this);
		return urlForm;
	}
	...
	public Application applicationScope() throws PageException {
		if(application == null) {
			if(!applicationContext.hasName())
				throw new ExpressionException("there is no application context defined for this application",
						hintAplication("you can define a application context"));
	...
	public Client clientScopeEL() {
		if(client == null) {
			if(applicationContext == null || !applicationContext.hasName())
				return null;
			if(!applicationContext.isSetClientManagement())
				return null;
	...
	/*
	 * public Object touch(Object coll, String key) throws PageException { Object o=getCollection(coll,key,null); if(o!=null) return o; return set(coll,key,new
	 * StructImpl()); }
	 */
	...
	/*
	 * private Object _touch(Scope scope, String key) throws PageException { Object o=scope.get(key,null); if(o!=null) return o; return scope.set(key, new
	 * StructImpl()); }
	 */
	...
	public void subparam(String type, String name, final Object value, double min, double max, String strPattern, int maxLength, final boolean isNew)
			...
			if("range".equals(type)) {
				...
				if(hasMin && number < min)
					throw new ExpressionException(
							"The number [" + Caster.toString(number) + "] is to small, the number must be at least [" + Caster.toString(min) + "]");

				if(hasMax && number > max)
					throw new ExpressionException(
							"The number [" + Caster.toString(number) + "] is to big, the number cannot be bigger than [" + Caster.toString(max) + "]");
	 ...
	 * 
	 * // check attributes name if(StringUtil.isEmpty(name)) throw new ExpressionException("The attribute name is required");
	 * 
	 * Object value=null; boolean isNew=false;
	 * 
	 * // get value value=VariableInterpreter.getVariableEL(this,name,NullSupportHelper.NULL(this)); if(NullSupportHelper.NULL(this)==value) {
	 * if(defaultValue==null) throw new ExpressionException("The required parameter ["+name+"] was not provided."); value=defaultValue; isNew=true; }
	 * 
	 * // cast and set value if(!"any".equals(type)) { // range if("range".equals(type)) { boolean hasMin=Decision.isValid(min); boolean
	 * hasMax=Decision.isValid(max); double number = Caster.toDoubleValue(value);
	 * 
	 ...
	 * 
	 * if(hasMin && number<min) throw new
	 * ExpressionException("The number ["+Caster.toString(number)+"] is to small, the number must be at least ["+Caster.toString(min)+"]");
	 * 
	 * if(hasMax && number>max) throw new
	 * ExpressionException("The number ["+Caster.toString(number)+"] is to big, the number cannot be bigger than ["+Caster.toString(max)+"]");
	 * 
	 * setVariable(name,Caster.toDouble(number)); } // regex else if("regex".equals(type) || "regular_expression".equals(type)) { String
	 ...
	 * 
	 * setVariable( name, value ); } else { if(!Decision.isCastableTo(type,value,true,true,maxLength)) { if(maxLength>-1 && ("email".equalsIgnoreCase(type) ||
	 * "url".equalsIgnoreCase(type) || "string".equalsIgnoreCase(type))) { StringBuilder msg=new StringBuilder(CasterException.createMessage(value, type));
	 * msg.append(" with a maximum length of "+maxLength+" characters"); throw new CasterException(msg.toString()); } throw new CasterException(value,type); }
	 * 
	 * setVariable(name,value); //REALCAST setVariable(name,Caster.castTo(this,type,value,true)); } } else if(isNew) setVariable(name,value); }
	 */
		...
		if(!Abort.isSilentAbort(pe)) {
			...
			if(ep != null) {
				...
				if(ps.physcalExists()) {
					...
					pe = new ApplicationException(
							"The error page template for type request only works if the actual source file also exists. If the exception file is in an "
			...
			try {
				...
				if(!Abort.isSilentAbort(pe))
					forceWrite(getConfig().getDefaultDumpWriter(DumpWriter.DEFAULT_RICH).toString(this, pe.toDumpData(this, 9999, DumpUtil.toDumpProperties()),
							true));
			}
			catch (Exception e) {
			}
	...
	public void outputStart() {
		outputState++;
		if(enablecfoutputonly > 0 && outputState == 1)
			writer = forceWriter;
	...
	public void setCFOutputOnly(boolean boolEnablecfoutputonly) {
		if(boolEnablecfoutputonly)
			this.enablecfoutputonly++;
			...
			if(restMappings != null)
				for (int i = 0; i < restMappings.length; i++) {
					...
					if(pathInfo.startsWith(m.getVirtualWithSlash(), 0) && m.getPhysical() != null) {
						...
						rl = new RestRequestListener(m, pathInfo.substring(m.getVirtual().length()), matrix, format, hasFormatExtension, accept, contentType,
								null);
						break;
					}
				}
	...
	private final void _execute(String realPath, boolean throwExcpetion, boolean onlyTopLevel) throws PageException {
		...
		if(realPath.startsWith("/mapping-")) {
			...
			if(base == null)
				base = PageSourceImpl.best(config.getPageSources(this, null, realPath, onlyTopLevel, false, true));
		}
		else
			base = PageSourceImpl.best(config.getPageSources(this, null, realPath, onlyTopLevel, false, true));
	...
	public Object getThreadScope(Collection.Key name, Object defaultValue) {
		if(threads == null)
			threads = new StructImpl();
		if(name.equalsIgnoreCase(KeyConstants._cfthread))
			return threads; // do not change this, this is used!
	...
	public boolean isTrusted(Page page) {
		...
		if(it == ConfigImpl.INSPECT_NEVER)
			return true;
	...
	public short getSessionType() {
		if(isGatewayContext())
			return Config.SESSION_TYPE_APPLICATION;
	...
	public DataSource getDataSource(String datasource, DataSource defaultValue) {
		DataSource ds = getApplicationContext() == null ? null : getApplicationContext().getDataSource(datasource, null);
		if(ds == null)
			ds = getConfig().getDataSource(datasource, defaultValue);
		return ds;
	}
	...
	public boolean getAllowCompression() {
		if(applicationContext == null)
			return config.allowCompression();
	...
	public Object getCachedWithin(int type) {
		if(applicationContext == null)
			return config.getCachedWithin(type);
