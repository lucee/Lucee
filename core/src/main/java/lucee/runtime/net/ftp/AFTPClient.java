...
public abstract class AFTPClient {
	 ...
	 *                If the FTP server prematurely closes the connection as a result of the client being idle or some other reason causing the server to send
	 *                FTP reply code 421. This exception may be caught either as an IOException or independently as itself.
	 ...
	 * 
	 * @param pathname
	 *            The file or directory to list. Since the server may or may not expand glob expressions, using them here is not recommended and may well cause
	 *            this method to fail. Also, some servers treat a leading '-' as being an option. To avoid this interpretation, use an absolute pathname or
	 *            prefix the pathname with ./ (unix style servers). Some servers may support "--" as meaning end of options, in which case "-- -xyz" should
	 *            work.
	 *
	 ...
	 *                Thrown if the parserKey parameter cannot be resolved by the selected parser factory. In the DefaultFTPEntryParserFactory, this will happen
	 *                when parserKey is neither the fully qualified class name of a class implementing the interface
	 *                org.apache.commons.net.ftp.FTPFileEntryParser nor a string containing one of the recognized keys mapping to such a parser or if class
	 *                loader security issues prevent its being loaded.
	 */
	 ...
	 *                If an I/O error occurs while actually transferring the file. The CopyStreamException allows you to determine the number of bytes
	 *                transferred and the IOException causing the error. This exception may be caught either as an IOException or independently as itself.
	 * @exception IOException
	 *                If an I/O error occurs while either sending a command to the server or receiving a reply from the server.
	 */
	...
	public abstract void init(InetAddress host, int port, String username, String password, String fingerprint, boolean stopOnError)
			throws SocketException, IOException;
