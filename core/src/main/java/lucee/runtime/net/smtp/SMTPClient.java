...
public final class SMTPClient implements Serializable {
	...
	public void setHost(String host) throws PageException {
		if(!StringUtil.isEmpty(host, true))
			this.host = ListUtil.toStringArray(ListUtil.listToArrayRemoveEmpty(host, ','));
	}
	...
	private MimeMessageAndSession createMimeMessage(lucee.runtime.config.Config config, String hostName, int port, String username, String password,
			long lifeTimesan, long idleTimespan, boolean tls, boolean ssl, boolean sendPartial, boolean newConnection, boolean userset)
		...
		/*
		 * - mixed -- alternative --- text --- related ---- html ---- inline image ---- inline image -- attachment -- attachment
		 * 
		 */
	...
	/*
	 * private static void addMailcaps(ClassLoader cl) { try { Class<?> cCM = cl.loadClass("javax.activation.CommandMap"); Method getDefaultCommandMap =
	 * cCM.getMethod("getDefaultCommandMap", CLASS_EMPTY); Object oMCM = getDefaultCommandMap.invoke(null, OBJECT_EMPTY);
	 * 
	 ...
	 * addMailcap(oMCM,addMailcap,"message/rfc822;;	x-java-content-handler=com.sun.mail.handlers.message_rfc822"); } catch(Throwable t)
	 * {ExceptionUtil.rethrowIfNecessary(t);} } private static void addMailcap(Object oMCM, Method addMailcap, String value) throws IllegalAccessException,
	 * IllegalArgumentException, InvocationTargetException { addMailcap.invoke(oMCM, new Object[]{value}); }
	 */
	...
	private void listener(ConfigWeb config, Server server, Log log, Exception e, long exe) {
		if(e == null)
			log.info("mail", "mail sent (subject:" + subject + "from:" + toString(from) + "; to:" + toString(tos) + "; cc:" + toString(ccs) + "; bcc:"
					+ toString(bccs) + "; ft:" + toString(fts) + "; rt:" + toString(rts) + ")");
		else
			LogUtil.log(log, Log.LEVEL_ERROR, "mail", e);
