...
public final class ReqRspUtil {
	...
	public static boolean isThis(HttpServletRequest req, URL url) {
		try {
			...
			if(urlPort <= 0)
				urlPort = HTTPUtil.isSecure(url) ? 443 : 80;
			if(reqPort <= 0)
			...
			if(reqHost.equalsIgnoreCase(urlHost))
				return true;
			if(IsLocalHost.invoke(reqHost) && IsLocalHost.invoke(reqHost))
				return true;

			InetAddress urlAddr = InetAddress.getByName(urlHost);

			InetAddress reqAddr = InetAddress.getByName(reqHost);
			if(reqAddr.getHostName().equalsIgnoreCase(urlAddr.getHostName()))
				return true;
			if(reqAddr.getHostAddress().equalsIgnoreCase(urlAddr.getHostAddress()))
				return true;
	...
	public static Object getRequestBody(PageContext pc, boolean deserialized, Object defaultValue) {
		...
		boolean isBinary = !(strContentType == null || HTTPUtil.isTextMimeType(contentType)
	...
	public static String getRootPath(ServletContext sc) {
		...
		if(root == null)
			throw new RuntimeException("cannot determinate webcontext root, the ServletContext from class [" + sc.getClass().getName()
	...
	public static String getDomain(HttpServletRequest req) { // DIFF 23
		...
		if(!StringUtil.isEmpty(req.getContextPath()))
