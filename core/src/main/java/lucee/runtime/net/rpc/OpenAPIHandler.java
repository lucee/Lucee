...
public class OpenAPIHandler {
	...
	public static void handleOpenAPI(PageContext pc, HttpServletRequest req, HttpServletResponse rsp) throws IOException {
		try {
			...
			/*
			PageSource ps = pc.getPageSource(cfcPath);
			if (!ps.exists()) {
				rsp.setStatus(HttpServletResponse.SC_NOT_FOUND);
				rsp.setContentType("application/json");
				rsp.getWriter().write("{\"error\":\"Component not found: " + cfcPath + CFC_EXTENSION + "\"}");
				return;
			}
			*/
			...
			if (!hasRemoteMethods(comp)) {
				...
				rsp.setContentType("application/json");
				rsp.getWriter().write("{\"error\":\"No remote methods found in component\"}");
				return;
			}
			...
			rsp.setContentType("application/json");
			rsp.getWriter().write("{\"error\":\"" + escapeJson(e.getMessage()) + "\"}");
		} catch (Exception e) {
			// Handle unexpected error
			rsp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			rsp.setContentType("application/json");
			rsp.getWriter().write("{\"error\":\"Internal server error: " + escapeJson(e.getMessage()) + "\"}");
		}
	}
	
	public static void handleSwaggerUI(PageContext pc, HttpServletRequest req, HttpServletResponse rsp) throws IOException {
		String cfcPath = req.getServletPath();
		String openApiUrl = req.getRequestURL() + "?openapi";
		
		String swaggerHtml = generateSwaggerHTML(openApiUrl, cfcPath);
	...
	private static String generateSwaggerHTML(String openApiUrl, String cfcPath) {
		return "<!DOCTYPE html>\n" +
			   "<html lang=\"en\">\n" +
			   "<head>\n" +
			   "  <meta charset=\"UTF-8\">\n" +
