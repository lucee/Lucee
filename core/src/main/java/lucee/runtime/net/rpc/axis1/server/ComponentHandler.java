...
public final class ComponentHandler extends BasicHandler {
	...
	/**
	 * handle all the work necessary set up the "proxy" RPC service surrounding
	...
	protected void setupService(MessageContext msgContext) throws Exception {
		...
		Class clazz = cfc.getJavaAccessClass(ThreadLocalPageContext.get(),
		...
		if (!isnew.toBooleanValue()
				&& (pair = (Pair) soapServices.get(clazzName)) != null) {
