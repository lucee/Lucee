...
public class WSHandlerReflector implements WSHandler {
	...
	public boolean isSOAPRequest() {
		try {
			if(isSOAPRequest==null) 
				isSOAPRequest=clazz.getMethod("isSOAPRequest", EMPTY_CLASS);
	...
	public String getTypeAsString() {
		try {
			if(getTypeAsString==null) 
				getTypeAsString=clazz.getMethod("getTypeAsString", EMPTY_CLASS);
	...
	public Class<?> toWSTypeClass(Class<?> _clazz) {
		try {
			if(toWSTypeClass==null) 
				toWSTypeClass=this.clazz.getMethod("toWSTypeClass", new Class[]{Class.class});
	...
	public WSServer getWSServer(PageContext pc) throws PageException {
		try {
			if(getWSServer==null) 
				getWSServer=clazz.getMethod("getWSServer", new Class[]{PageContext.class,});
	...
	public WSClient getWSClient(String wsdlUrl, String username, String password, ProxyData proxyData) throws PageException {
		try {
			if(getWSClient==null) 
				getWSClient=clazz.getMethod("getWSClient", new Class[]{String.class,String.class,String.class,ProxyData.class,});
