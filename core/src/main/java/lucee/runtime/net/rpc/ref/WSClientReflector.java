...
public class WSClientReflector implements WSClient {
	...
	public Call getLastCall() throws PageException {
		try {
			if(getLastCall==null) 
				getLastCall=clazz.getMethod("getLastCall", WSHandlerReflector.EMPTY_CLASS);
			return (Call) getLastCall.invoke(obj,WSHandlerReflector.EMPTY_OBJECT);
		}
		catch(Exception e) {
			throw Caster.toPageException(e);
		}
	...
	public Object callWithNamedValues(Config config, Key methodName, Struct arguments) throws PageException {
		try {
			if(callWithNamedValues==null) 
				callWithNamedValues=clazz.getMethod("callWithNamedValues", new Class[]{Config.class,Key.class,Struct.class});
	...
	public Node getSOAPRequest() throws PageException {
		try {
			if(getSOAPRequest==null) 
				getSOAPRequest=clazz.getMethod("getSOAPRequest", WSHandlerReflector.EMPTY_CLASS);
	...
	public Node getSOAPResponse() throws PageException {
		try {
			if(getSOAPResponse==null) 
				getSOAPResponse=clazz.getMethod("getSOAPResponse", WSHandlerReflector.EMPTY_CLASS);
	...
	public WSHandler getWSHandler() {
		try {
			if(getWSHandler==null) 
				getWSHandler=clazz.getMethod("getWSHandler", WSHandlerReflector.EMPTY_CLASS);
