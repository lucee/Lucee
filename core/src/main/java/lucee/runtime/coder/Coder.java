...
public final class Coder {
	...
	public static byte[] decode(String type, String value) throws CoderException {
		...
		if(type.equals("hex"))
			return decode(ENCODING_HEX, value);
		if(type.equals("uu"))
			return decode(ENCODING_UU, value);
		if(type.equals("base64"))
			return decode(ENCODING_BASE64, value);
	...
	public static byte[] decode(short type, String value) throws CoderException {
		if(type == ENCODING_UU)
			return UUCoder.decode(value);
		else if(type == ENCODING_HEX)
			return HexCoder.decode(value);
		else if(type == ENCODING_BASE64)
	...
	public static String encode(String type, byte[] value) throws CoderException {
		type = type.toLowerCase().trim();
		if(type.equals("hex"))
			return encode(ENCODING_HEX, value);
		if(type.equals("uu"))
			return encode(ENCODING_UU, value);
		if(type.equals("base64"))
			return encode(ENCODING_BASE64, value);
		throw new CoderException("Invalid encoding definition [" + type + "]. Valid encodings are [hex, uu, base64].");
	}
	...
	public static String encode(short type, byte[] value) throws CoderException {
		if(type == ENCODING_UU)
			return UUCoder.encode(value);
		else if(type == ENCODING_HEX)
			return HexCoder.encode(value);
		else if(type == ENCODING_BASE64)
