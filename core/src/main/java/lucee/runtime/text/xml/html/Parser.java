...
public class Parser extends DefaultHandler implements ScanHandler, XMLReader, LexicalHandler {
	...
	/**
	 * A value of "true" indicates namespace URIs and unprefixed local names for element and attribute
	 * names will be available.
	 **/
	public final static String namespacesFeature = "http://xml.org/sax/features/namespaces";

	/**
	 * A value of "true" indicates that XML qualified names (with prefixes) and attributes (including
	 * xmlns* attributes) will be available. We don't support this value.
	 **/
	...
	/**
	 * May be examined only during a parse, after the startDocument() callback has been completed;
	 * read-only. The value is true if the document specified standalone="yes" in its XML declaration,
	...
	/**
	 * A value of "true" indicates that the LexicalHandler will report the beginning and end of
	 * parameter entities (it won't).
	 **/
	...
	/**
	 * A value of "true" indicates that system IDs in declarations will be absolutized (relative to
	 * their base URIs) before reporting. (This returns true but doesn't actually do anything.)
	 **/
	public final static String resolveDTDURIsFeature = "http://xml.org/sax/features/resolve-dtd-uris";

	/**
	 * Has a value of "true" if all XML names (for elements, prefixes, attributes, entities, notations,
	 * and local names), as well as Namespace URIs, will have been interned using
	 * java.lang.String.intern. This supports fast testing of equality/inequality against string
	 * constants, rather than forcing slower calls to String.equals(). (We always intern.)
	 **/
	 ...
	 * ContentHandler.setDocumentLocator() implement the org.xml.sax.ext.Locator2 interface. (They
	 * don't.)
	 **/
	public final static String useLocator2Feature = "http://xml.org/sax/features/use-locator2";

	/**
	 * Returns "true" if, when setEntityResolver is given an object implementing the
	 * org.xml.sax.ext.EntityResolver2 interface, those new methods will be used. (They won't be.)
	 **/
	public final static String useEntityResolver2Feature = "http://xml.org/sax/features/use-entity-resolver2";

	/**
	 * Controls whether the parser is reporting all validity errors (We don't report any validity
	 * errors.)
	 **/
	 ...
	 * Appendix B of the XML 1.1 Recommendation. (We don't normalize.)
	 **/
	 ...
	 * declaration attributes as being in the http://www.w3.org/2000/xmlns/ namespace. (It doesn't.)
	 **/
	...
	/**
	 * A value of "true" indicates that the parser will give unknown elements a content model of EMPTY;
	 * a value of "false", a content model of ANY.
	 **/
	...
	/**
	 * A value of "true" indicates that the parser will return default attribute values for missing
	...
	/**
	 * A value of "true" indicates that the parser will transmit whitespace in element-only content via
	 * the SAX ignorableWhitespace callback. Normally this is not done, because HTML is an SGML
	 * application and SGML suppresses such whitespace.
	 **/
	public final static String ignorableWhitespaceFeature = "http://www.ccil.org/~cowan/tagsoup/features/ignorable-whitespace";

	/**
	 * A value of "true" indicates that the parser will treat CDATA elements specially. Normally true,
	 * since the input is by default HTML.
	 **/
	public final static String CDATAElementsFeature = "http://www.ccil.org/~cowan/tagsoup/features/cdata-elements";

	/**
	 * Used to see some syntax events that are essential in some applications: comments, CDATA
	 * delimiters, selected general entity inclusions, and the start and end of the DTD (and declaration
	...
	private Reader getReader(InputSource s) throws IOException {
	...
	/**
	 * Parsing the complete XML Document Type Definition is way too complex, but for many simple cases
	 * we can extract something useful from it.
	 *
	 * doctypedecl ::= '<!DOCTYPE' S Name (S ExternalID)? S? ('[' intSubset ']' S?)? '>' DeclSep ::=
	 * PEReference | S intSubset ::= (markupdecl | DeclSep)* markupdecl ::= elementdecl | AttlistDecl |
	 * EntityDecl | NotationDecl | PI | Comment ExternalID ::= 'SYSTEM' S SystemLiteral | 'PUBLIC' S
