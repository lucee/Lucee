...
public class ScriptEngineFactoryImpl implements ScriptEngineFactory {
	...
	public List<String> getNames() {
		return ListUtil.arrayToList(
				dialect==CFMLEngine.DIALECT_CFML?Constants.CFML_ALIAS_NAMES:Constants.LUCEE_ALIAS_NAMES);
	}
	...
	public Object getParameter(String key) {
		...
		if(key.equalsIgnoreCase(ScriptEngine.NAME))
			return ConfigWebUtil.toDialect(dialect, "");
		
		if(key.equalsIgnoreCase(ScriptEngine.ENGINE))
			return Constants.NAME+" (dialect:"+ConfigWebUtil.toDialect(dialect, "")+")";
		
		if(key.equalsIgnoreCase(ScriptEngine.ENGINE_VERSION) || key.equalsIgnoreCase(ScriptEngine.LANGUAGE_VERSION))
