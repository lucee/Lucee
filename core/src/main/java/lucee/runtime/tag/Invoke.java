...
public final class Invoke extends BodyTagImpl implements DynamicAttributes {
	...
	private void doWebService(String webservice) throws PageException {
		...
		WSClient ws = username != null ? ((ConfigWebImpl) ThreadLocalPageContext.getConfig()).getWSHandler().getWSClient(webservice, username, password, pd)
				: ((ConfigWebImpl) ThreadLocalPageContext.getConfig()).getWSHandler().getWSClient(webservice, null, null, pd);
