...
/**
 * Defines scoping for a CFML application, enables or disables storing client variables, and specifies a client variable storage mechanism. By default, client
 * variables are disabled. Also, enables session variables and sets timeouts for session and application variables. Session and application variables are stored
 * in memory.
 *
 *
 *
...
public final class Application extends TagImpl {
	...
	/**
	 * set the value setclientcookies Yes or No. Yes enables client cookies. Default is Yes. If you set this attribute to "No", CFML does not automatically send
	 * the CFID and CFTOKEN cookies to the client browser; you must manually code CFID and CFTOKEN on the URL for every page that uses Session or Client
	 * variables.
	 * 
	 * @param setClientCookies
	 *            value to set
	 **/
	...
	/**
	 * set the value setdomaincookies Yes or No. Sets the CFID and CFTOKEN cookies for a domain, not just a single host. Applications that are running on
	 * clusters must set this value to Yes. The default is No.
	 * 
	 * @param setDomainCookies
	 *            value to set
	 **/
	...
	/**
	 * set the value sessiontimeout Enter the CreateTimeSpan function and values in days, hours, minutes, and seconds, separated by commas, to specify the
	...
	/**
	 * set the value applicationtimeout Enter the CreateTimeSpan function and values in days, hours, minutes, and seconds, separated by commas, to specify the
	...
	/**
	 * set the value name The name of your application. This name can be up to 64 characters long. Required for application and session variables, optional for
	 * client variables
	 * 
	 * @param name
	 *            value to set
	...
	public void setAction(String strAction) throws ApplicationException {
		...
		if(strAction.equals("create"))
			...
			throw new ApplicationException("invalid action definition [" + strAction + "] for tag application, valid values are [create,update]");
	...
	public void setLoginstorage(String loginstorage) throws ApplicationException {
		...
		if(loginstorage.equals("session"))
			this.loginstorage = Scope.SCOPE_SESSION;
		else if(loginstorage.equals("cookie"))
			this.loginstorage = Scope.SCOPE_COOKIE;
		else
			throw new ApplicationException("invalid loginStorage definition [" + loginstorage + "] for tag application, valid values are [session,cookie]");
	}

	/**
	 * @param scriptrotect
	 *            the scriptrotect to set
	 */
	...
	private boolean set(ApplicationContext ac, boolean update) throws PageException {
		...
		if(ftp != null)
			((ApplicationContextSupport)ac).setFTP(AppListenerUtil.toFTP(ftp));
