...
public final class Ftp extends TagImpl {
	...
	private AFTPClient actionGetFile() throws PageException, IOException {
		...
		if (failifexists && local.exists()) throw new ApplicationException("File [" + local + "] already exists, if you want to overwrite, set attribute [failIfExists] to false");
	...
	private void required(String attributeName, String atttributValue) throws ApplicationException {
		if (atttributValue == null)
			throw new ApplicationException("invalid attribute combination for the tag ftp", "attribute [" + attributeName + "] is required, if action is [" + action + "]");
	}
