...
public final class Ftp extends TagImpl {
	...
	/*
	 * private static final Key = KeyImpl.getInstance(); private static final Key = KeyImpl.getInstance(); private static final Key = KeyImpl.getInstance();
	 * private static final Key = KeyImpl.getInstance(); private static final Key = KeyImpl.getInstance(); private static final Key = KeyImpl.getInstance();
	 */
		...
		do {
			try {
				...
				// else if(action.equals("copy")) client=actionCopy();
					...
					throw new ApplicationException("attribute action has an invalid value [" + action + "]",
	 ...
	 * @throws IOException
	 * 
	 * @throws PageException / private FTPFile exists(FTPClient client, String strPath) throws PageException, IOException { strPath=strPath.trim();
	 * 
	 * // get parent path FTPPath path=new FTPPath(client.printWorkingDirectory(),strPath); String name=path.getName(); print.out("path:"+name);
	 * 
	 * // when directory FTPFile[] files=null; try { files = client.listFiles(path.getPath()); } catch (IOException e) {}
	 * 
	 * if(files!=null) { for(int i=0;i<files.length;i++) { if(files[i].getName().equalsIgnoreCase(name)) { return files[i]; } }
	 * 
	 * } return null; }
	 */
	...
	private AFTPClient actionPutFile() throws IOException, PageException {
		...
		// if(failifexists && local.exists()) throw new ApplicationException("File ["+local+"] already exist, if you want to overwrite, set attribute
	...
	private void required(String attributeName, String atttributValue) throws ApplicationException {
		if(atttributValue == null)
			throw new ApplicationException("invalid attribute constelation for the tag ftp",
