...
/**
 * implements a JSP Factory, this class produces JSP compatible PageContext objects, as well as the required ColdFusion specified interfaces
 */
public final class CFMLFactoryImpl extends CFMLFactory {
	...
	@Override
	public javax.servlet.jsp.PageContext getPageContext(Servlet servlet, ServletRequest req, ServletResponse rsp, String errorPageURL, boolean needsSession,
			int bufferSize, boolean autoflush) {
		return getPageContextImpl((HttpServlet)servlet, (HttpServletRequest)req, (HttpServletResponse)rsp, errorPageURL, needsSession, bufferSize, autoflush,
				true, false, -1, true, false, false);
	}

	@Override
	@Deprecated
	public PageContext getLuceePageContext(HttpServlet servlet, HttpServletRequest req, HttpServletResponse rsp, String errorPageURL, boolean needsSession,
			int bufferSize, boolean autoflush) {
		// runningCount++;
	...
	@Override
	public PageContext getLuceePageContext(HttpServlet servlet, HttpServletRequest req, HttpServletResponse rsp, String errorPageURL, boolean needsSession,
			int bufferSize, boolean autoflush, boolean register, long timeout, boolean register2RunningThreads, boolean ignoreScopes) {
		// runningCount++;
	...
	public PageContextImpl getPageContextImpl(HttpServlet servlet, HttpServletRequest req, HttpServletResponse rsp, String errorPageURL, boolean needsSession,
			int bufferSize, boolean autoflush, boolean register2Thread, boolean isChild, long timeout, boolean register2RunningThreads, boolean ignoreScopes,
			boolean createNew) {
		PageContextImpl pc;
		...
		if(register2RunningThreads) {
			...
			if(isChild)
				runningChildPcs.put(Integer.valueOf(pc.getId()), pc);
	...
	public HttpServlet getServlet() {
		if(_servlet == null)
			_servlet = new HTTPServletImpl(servletConfig, servletConfig.getServletContext(), servletConfig.getServletName());
		;
		return _servlet;
	}
	...
	public int toDialect(String ext) {
		...
		if(cfmlExtensions == null)
			_initExtensions();
		if(cfmlExtensions.contains(ext.toLowerCase()))
	...
	public int toDialect(String ext, int defaultValue) {
		...
		if(cfmlExtensions == null)
			_initExtensions();
		if(cfmlExtensions.contains(ext = ext.toLowerCase()))
			return CFMLEngine.DIALECT_CFML;
		if(luceeExtensions.contains(ext))
