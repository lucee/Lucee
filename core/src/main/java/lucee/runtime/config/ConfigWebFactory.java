...
public final class ConfigWebFactory extends ConfigFactory {
	...
	public static ConfigWebPro newInstanceSingle(CFMLEngine engine, CFMLFactoryImpl factory, ConfigServerImpl configServer, Resource configDirWeb, ServletConfig servletConfig,
		...
		ConfigWebPro configWeb = existingToUpdate != null ? existingToUpdate.update(factory, configServer, servletConfig, configDirWeb)
	 ...
	 * @throws NoSuchAlgorithmException
	 */ // MUST
	public static void reloadInstance(CFMLEngine engine, ConfigServer cs, ConfigWebImpl cwi, boolean force)
	...
	synchronized static void load(ConfigServerImpl config, Struct root, boolean isReload, boolean doNew, boolean essentialOnly) throws IOException {
		if (LOG) LogUtil.logGlobal(config, Log.LEVEL_INFO, ConfigWebFactory.class.getName(), "start reading config");
		...
		try {
			...
			if (LOG) LogUtil.logGlobal(config, Log.LEVEL_DEBUG, ConfigWebFactory.class.getName(), "fixed salt");

			// delete to big felix.log (there is also code in the loader to do this, but if the loader is not
			// updated ...)
			if (!essentialOnly) {
				try {
					File rr = config.getEngine().getCFMLEngineFactory().getResourceRoot();
	...
	private static Struct reload(Struct root, ConfigImpl config) throws PageException, IOException, ConverterException {
		// store as json
	...
	private static void settings(ConfigServerImpl config, Log log) {
		try {
	...
	private static void doCheckChangesInLibraries(ConfigServerImpl config) {
		...
		config.setLibHash(HashUtil.create64BitHashAsString(sb.toString()));
					...
					while (it.hasNext()) {
						try {
							...
							if (listener != null || listenerMode != -1) {
								...
								else if (listener == null) listener = ConfigUtil.loadListener(ConfigUtil.toListenerType(config.getApplicationListener().getType(), -1), null);
	...
	private static void _loadLoggers(ConfigImpl config, Struct root, boolean isReload) {
		...
		try {
			...
			// main logger
			String mainLogger = ConfigUtil.getAsString("mainLogger", root, null);
	...
	private static void _loadCache(ConfigImpl config, Struct root, Log log) {
		try {
			...
			{
				Struct eCaches = ConfigUtil.getAsStruct("caches", root);
	...
	private static void _loadConfig(ConfigImpl config, Struct root) {
		...
		if (true) {
			...
			if (!StringUtil.isEmpty(keyList)) {
				...
				((ConfigServerImpl) config).setAuthenticationKeys(keys);
			}
		}
	...
	private static void _loadTag(ConfigImpl config, Struct root, Log log) {
		try {
			{
				Array tags = ConfigUtil.getAsArray("tags", root);
				Struct tag;
				ClassDefinition cd;
				String nss, ns, n;
				if (tags != null) {
			...
			Array defaults = ConfigUtil.getAsArray("tagDefaults", root);
			if (defaults.size() > 0) {
				Struct def;
	...
	private static void _loadTempDirectory(ConfigImpl config, Struct root, boolean isReload, Log log) {
		try {
			...
			String strTempDirectory = ConfigUtil.translateOldPath(getAttr(root, "tempDirectory"));

			Resource cst = null;
			// Temp Dir
			if (!StringUtil.isEmpty(strTempDirectory))
				cst = ConfigUtil.getFile(configDir, strTempDirectory, null, configDir, FileUtil.TYPE_DIR, ResourceUtil.LEVEL_GRAND_PARENT_FILE, config);

			if (cst == null) cst = ConfigUtil.getFile(configDir, "temp", null, configDir, FileUtil.TYPE_DIR, ResourceUtil.LEVEL_GRAND_PARENT_FILE, config);
	...
	private static void _loadFilesystem(ConfigImpl config, Struct root, boolean doNew, Log log) {
		try {
			...
			if (fileSystem != null) {
				...
				strDeployDirectory = ConfigUtil.translateOldPath(getAttr(fileSystem, "deployDirectory"));
			...
			// Deploy Dir
			Resource dd = ConfigUtil.getFile(configDir, strDeployDirectory, "cfclasses", configDir, FileUtil.TYPE_DIR, ResourceUtil.LEVEL_GRAND_PARENT_FILE, config);
	...
	private static void _loadRemoteClient(ConfigImpl config, Struct root, Log log) {
		try {
			...
			Resource file = ConfigUtil.getFile(config.getRootDirectory(), strDir, "client-task", config.getConfigDir(), FileUtil.TYPE_DIR, ResourceUtil.LEVEL_GRAND_PARENT_FILE,
					config);
	...
	private static void _loadORM(ConfigImpl config, Struct root, Log log) {
		try {
			...
			// config
			ORMConfiguration ormConfig = root == null ? null : ORMConfigurationImpl.load(config, null, orm, config.getRootDirectory(), null);
		...
		try {
			...
			if (mode == ConfigPro.MODE_STRICT) {
				...
				if (hasAccess && !StringUtil.isEmpty(strScopeCascadingType)) {
					config.setScopeCascadingType(ConfigUtil.toScopeCascading(strScopeCascadingType, Config.SCOPE_STANDARD));
				}
			}
			...
			if (hasAccess && !StringUtil.isEmpty(strClientDirectory)) {
				strClientDirectory = ConfigUtil.translateOldPath(strClientDirectory);
				Resource res = ConfigUtil.getFile(configDir, strClientDirectory, "client-scope", configDir, FileUtil.TYPE_DIR, ResourceUtil.LEVEL_PARENT_FILE, config);
	...
	private static void _loadJava(ConfigImpl config, Struct root, Log log) {
		try {
			...
			if (!StringUtil.isEmpty(strInspectTemplate, true)) {
				config.setInspectTemplate(ConfigUtil.inspectTemplate(strInspectTemplate, ConfigPro.INSPECT_AUTO));
				...
				while (it.hasNext()) {
					try {
						...
						list.put(id, new DebugEntry(id, getAttr(e, "type"), getAttr(e, "iprange"), getAttr(e, "label"), getAttr(e, "path"), getAttr(e, "fullname"),
								ConfigUtil.getAsStruct(e, true, "custom")));
					}
					catch (Throwable t) {
						ExceptionUtil.rethrowIfNecessary(t);
						log(config, log, t);
	...
	private static void _loadExtensionBundles(ConfigImpl config, Struct root, Log log) {
		...
		try {
			...
			if (config instanceof ConfigServer) {
				changed = ConfigFactory.modeChange(config.getConfigDir(), "single", false);
			}
	...
	private static void _loadApplication(ConfigImpl config, Struct root, int mode, Log log) {
		try {
			...
			if (hasAccess && !StringUtil.isEmpty(strCacheDirectory)) {
				...
				Resource res = ConfigUtil.getFile(configDir, strCacheDirectory, "cache", configDir, FileUtil.TYPE_DIR, ResourceUtil.LEVEL_GRAND_PARENT_FILE, config);
