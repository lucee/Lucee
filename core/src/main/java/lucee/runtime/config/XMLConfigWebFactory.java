...
public final class XMLConfigWebFactory extends XMLConfigFactory {
	...
	synchronized static void load(ConfigServerImpl cs, ConfigImpl config, Document doc, boolean isReload, boolean doNew) throws IOException {
		...
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "deploy web context");
		loadConfig(cs, config, doc);
		int mode = config.getMode();
		Log log = config.getLog("application");
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded config");
		loadConstants(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded constants");
		loadLoggers(cs, config, doc, isReload, log);
		log = config.getLog("application");
		// loadServerLibDesc(cs, config, doc,log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded loggers");
		loadTempDirectory(cs, config, doc, isReload, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded temp dir");
		loadId(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded id");
		loadVersion(config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded version");
		loadSecurity(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded security");
		try {
		...
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded lib");
		loadSystem(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded system");
		loadResourceProvider(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded resource providers");
		loadFilesystem(cs, config, doc, doNew, log); // load this before execute any code, what for example loadxtension does (json)
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded filesystem");
		loadExtensionBundles(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded extension bundles");
		loadWS(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded webservice");
		loadORM(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded orm");
		loadCacheHandler(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded cache handlers");
		loadCharset(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded charset");
		loadApplication(cs, config, doc, mode, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded application");
		loadMappings(cs, config, doc, mode, log); // it is important this runs after
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded mappings");
		// loadApplication
		loadRest(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded rest");
		loadExtensions(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded extensions");
		loadPagePool(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded page pool");
		loadDataSources(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded datasources");
		loadCache(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded cache");
		loadCustomTagsMappings(cs, config, doc, mode, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded custom tag mappings");
		// loadFilesystem(cs, config, doc, doNew); // load tlds
		loadTag(cs, config, doc, log); // load tlds
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded tags");
		loadRegional(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded regional");
		loadCompiler(cs, config, doc, mode, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded compiler");
		loadScope(cs, config, doc, mode, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded scope");
		loadMail(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded mail");
		loadSearch(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded search");
		loadScheduler(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded scheduled tasks");
		loadDebug(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded debug");
		loadError(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded error");
		loadCFX(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded cfx");
		loadComponent(cs, config, doc, mode, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded component");
		loadUpdate(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded update");
		loadJava(cs, config, doc, log); // define compile type
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded java");
		loadSetting(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded setting");
		loadProxy(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded proxy");
		loadRemoteClient(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded remote clients");
		loadVideo(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded video");
		loadFlex(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded flex");
		settings(config, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded settings2");
		loadListener(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded listeners");
		loadDumpWriter(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded dump writers");
		loadGatewayEL(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded gateways");
		loadExeLog(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded exe log");
		loadQueue(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded queue");
		loadMonitors(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded monitors");
		loadLogin(cs, config, doc, log);
		if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "loaded login");
		config.setLoadTime(System.currentTimeMillis());

		if (config instanceof ConfigWebImpl) {
			TagUtil.addTagMetaData((ConfigWebImpl) config, log);
			if (LOG) LogUtil.logGlobal(ThreadLocalPageContext.getConfig(cs == null ? config : cs), Log.LEVEL_DEBUG, XMLConfigWebFactory.class.getName(), "added tag meta data");
		}
	}
		...
		try {
			...
			for (int i = 0; i < types.length; i++) {
				...
				if (hasAccess && !StringUtil.isEmpty(def)) {
					...
					if (eCache.hasAttribute("default-" + typeNames[i])) config.setCacheDefaultConnectionName(types[i], "");
	...
	private static void loadFilesystem(ConfigServerImpl configServer, ConfigImpl config, Document doc, boolean doNew, Log log) {
		try {
			...
			// String strFLDDirectory =
			// hasCS?null:SystemUtil.getSystemPropOrEnvVar("lucee.library.addional.fld", null);
			// String strTLDDirectory =
			// hasCS?null:SystemUtil.getSystemPropOrEnvVar("lucee.library.addional.tld", null);
			...
			if (fileSystem != null) {
				...
				// if(StringUtil.isEmpty(strTLDDirectory)) strTLDDirectory =
				// ConfigWebUtil.translateOldPath(fileSystem.getAttribute("tld-addional-directory"));
				// if(StringUtil.isEmpty(strFLDDirectory)) strFLDDirectory =
				// ConfigWebUtil.translateOldPath(fileSystem.getAttribute("fld-addional-directory"));
	...
	private static void loadScope(ConfigServerImpl configServer, ConfigImpl config, Document doc, int mode, Log log) {
		try {
			...
			// Session-Storage
			{
				...
				if (StringUtil.isEmpty(sessionStorage, true)) sessionStorage = getAttr(scope, "session-storage");
