...
public final class AppListenerUtil {
	...
	public static PageSource getApplicationPageSource(PageContext pc, PageSource requestedPage, String filename, int mode) {
		if(mode == ApplicationListener.MODE_CURRENT)
			return getApplicationPageSourceCurrent(requestedPage, filename);
		if(mode == ApplicationListener.MODE_ROOT)
			return getApplicationPageSourceRoot(pc, filename);
	...
	public static String toStringType(ApplicationListener listener) {
		if(listener instanceof NoneAppListener)
			return "none";
		else if(listener instanceof MixedAppListener)
			return "mixed";
		else if(listener instanceof ClassicAppListener)
			return "classic";
		else if(listener instanceof ModernAppListener)
			return "modern";
		return "";
	}
	...
	public static DataSource toDataSource(Config config, String name, Struct data, Log log) throws PageException {
		...
		if(oConnStr != null) {
			...
			try {
				return ApplicationDataSource.getInstance(config, name, cd, Caster.toString(oConnStr), user, pass,
						Caster.toBooleanValue(data.get(BLOB, null), false), Caster.toBooleanValue(data.get(CLOB, null), false),
						Caster.toIntValue(data.get(CONNECTION_LIMIT, null), -1), Caster.toIntValue(data.get(CONNECTION_TIMEOUT, null), 1),
						Caster.toLongValue(data.get(META_CACHE_TIMEOUT, null), 60000L), Caster.toTimeZone(data.get(TIMEZONE, null), null),
						Caster.toIntValue(data.get(ALLOW, null), DataSource.ALLOW_ALL), Caster.toBooleanValue(data.get(STORAGE, null), false),
		...
		try {
			return new DataSourceImpl(config, name, dbt.classDefinition, Caster.toString(data.get(KeyConstants._host)), dbt.connectionString,
					Caster.toString(data.get(DATABASE)), Caster.toIntValue(data.get(KeyConstants._port, null), -1), user, pass,
					Caster.toIntValue(data.get(CONNECTION_LIMIT, null), -1), Caster.toIntValue(data.get(CONNECTION_TIMEOUT, null), 1),
					Caster.toLongValue(data.get(META_CACHE_TIMEOUT, null), 60000L), Caster.toBooleanValue(data.get(BLOB, null), false),
					Caster.toBooleanValue(data.get(CLOB, null), false), DataSource.ALLOW_ALL,
					Caster.toStruct(data.get(KeyConstants._custom, null), null, false), Caster.toBooleanValue(data.get(READ_ONLY, null), false), true,
					Caster.toBooleanValue(data.get(STORAGE, null), false), Caster.toTimeZone(data.get(TIMEZONE, null), null), "",
					ParamSyntax.toParamSyntax(data, ParamSyntax.DEFAULT), Caster.toBooleanValue(data.get("literalTimestampWithTSOffset", null), false),
					Caster.toBooleanValue(data.get("alwaysSetTimeout", null), false), log);
		}
		catch (Exception cnfe) {
			throw Caster.toPageException(cnfe);
		}
	...
	private static MappingData toMappingData(Object value, Resource source) throws PageException {
		...
		if(Decision.isStruct(value)) {
			...
			if(archive != null && physical != null) {
				...
				if(primary != null && primary.trim().equalsIgnoreCase("archive"))
					md.physicalFirst = false;
			}
			// only a archive
	...
	public static String toLocalMode(int mode, String defaultValue) {
		if(Undefined.MODE_LOCAL_OR_ARGUMENTS_ALWAYS == mode)
			return "modern";
		if(Undefined.MODE_LOCAL_OR_ARGUMENTS_ONLY_WHEN_EXISTS == mode)
			return "classic";
		return defaultValue;
	}
	...
	public static String toSessionType(short type, String defaultValue) {
		if(type == Config.SESSION_TYPE_APPLICATION)
			return "application";
		if(type == Config.SESSION_TYPE_JEE)
			return "jee";
		return defaultValue;
	}
	...
	public static short toSessionType(String str, short defaultValue) {
		if(!StringUtil.isEmpty(str, true)) {
			...
			if("j2ee".equals(str))
				return Config.SESSION_TYPE_JEE;
	...
	private static Properties toS3(String accessKeyId, String awsSecretKey, String defaultLocation, String host, String acl, TimeSpan cache) {
		...
		if(!StringUtil.isEmpty(awsSecretKey))
			s3.setSecretAccessKey(awsSecretKey);
		if(!StringUtil.isEmpty(defaultLocation))
			s3.setDefaultLocation(defaultLocation);
		if(!StringUtil.isEmpty(host))
			s3.setHost(host);
	...
	public static String translateScriptProtect(int scriptProtect) {
		if(scriptProtect == ApplicationContext.SCRIPT_PROTECT_NONE)
			return "none";
		if(scriptProtect == ApplicationContext.SCRIPT_PROTECT_ALL)
			return "all";

		Array arr = new ArrayImpl();
		if((scriptProtect & ApplicationContext.SCRIPT_PROTECT_CGI) > 0)
			arr.appendEL("cgi");
		if((scriptProtect & ApplicationContext.SCRIPT_PROTECT_COOKIE) > 0)
			arr.appendEL("cookie");
		if((scriptProtect & ApplicationContext.SCRIPT_PROTECT_FORM) > 0)
			arr.appendEL("form");
		if((scriptProtect & ApplicationContext.SCRIPT_PROTECT_URL) > 0)
			arr.appendEL("url");
	...
	public static int translateScriptProtect(String strScriptProtect) {
		...
		if("none".equals(strScriptProtect))
			return ApplicationContext.SCRIPT_PROTECT_NONE;
		if("no".equals(strScriptProtect))
			return ApplicationContext.SCRIPT_PROTECT_NONE;
		if("false".equals(strScriptProtect))
			return ApplicationContext.SCRIPT_PROTECT_NONE;

		if("all".equals(strScriptProtect))
			return ApplicationContext.SCRIPT_PROTECT_ALL;
		if("true".equals(strScriptProtect))
			return ApplicationContext.SCRIPT_PROTECT_ALL;
		if("yes".equals(strScriptProtect))
			return ApplicationContext.SCRIPT_PROTECT_ALL;
	...
	public static String translateLoginStorage(int loginStorage) {
		if(loginStorage == Scope.SCOPE_SESSION)
			return "session";
		return "cookie";
	}
	...
	public static int translateLoginStorage(String strLoginStorage, int defaultValue) {
		...
		if(strLoginStorage.equals("session"))
			return Scope.SCOPE_SESSION;
		if(strLoginStorage.equals("cookie"))
			return Scope.SCOPE_COOKIE;
		return defaultValue;
	}
	...
	public static String toWSType(short wstype, String defaultValue) {
		if(ApplicationContext.WS_TYPE_AXIS1 == wstype)
			return "Axis1";
		if(ApplicationContext.WS_TYPE_JAX_WS == wstype)
			return "JAX-WS";
		if(ApplicationContext.WS_TYPE_CXF == wstype)
			return "CXF";
		return defaultValue;
	}
	...
	public static short toWSType(String wstype, short defaultValue) {
		...
		/*
		 * if("jax".equalsIgnoreCase(wstype) || "jaxws".equalsIgnoreCase(wstype) || "jax-ws".equalsIgnoreCase(wstype)) return
		 * ApplicationContextPro.WS_TYPE_JAX_WS; if("cxf".equalsIgnoreCase(wstype)) return ApplicationContextPro.WS_TYPE_CXF;
		 */
		return defaultValue;
	}
	...
	public static int toCachedWithinType(String type, int defaultValue) {
		...
		if("function".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_FUNCTION;
		if("udf".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_FUNCTION;
		if("include".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_INCLUDE;
		if("query".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_QUERY;
		if("resource".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_RESOURCE;
		if("http".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_HTTP;
		if("file".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_FILE;
		if("webservice".equalsIgnoreCase(type))
			return Config.CACHEDWITHIN_WEBSERVICE;
	...
	public static String toCachedWithinType(int type, String defaultValue) {
		...
		if(type == Config.CACHEDWITHIN_FUNCTION)
			return "function";
		if(type == Config.CACHEDWITHIN_INCLUDE)
			return "include";
		if(type == Config.CACHEDWITHIN_QUERY)
			return "query";
		if(type == Config.CACHEDWITHIN_RESOURCE)
			return "resource";
		if(type == Config.CACHEDWITHIN_HTTP)
			return "http";
		if(type == Config.CACHEDWITHIN_FILE)
			return "file";
		if(type == Config.CACHEDWITHIN_WEBSERVICE)
			return "webservice";
	...
	public static SessionCookieData toSessionCookie(ConfigWeb config, Struct data) {
		if(data == null)
			return SessionCookieDataImpl.DEFAULT;
	...
	public static AuthCookieData toAuthCookie(ConfigWeb config, Struct data) {
		if(data == null)
			return AuthCookieDataImpl.DEFAULT;
	...
	private static TimeSpan toTimespan(Object obj, TimeSpan defaultValue) {
		if(!(obj instanceof TimeSpan)) {
			...
			if(tmp != null && tmp.doubleValue() <= 0d)
				return TimeSpanImpl.fromMillis(CookieImpl.NEVER * 1000);
		}
	...
	public static FTPConnectionData toFTP(Struct sct) {
		...
		if(o == null)
			o = sct.get(KeyConstants._user, null);
		...
		if(o == null)
			o = sct.get(KeyConstants._pass, null);
		...
		if(o == null)
			o = sct.get(KeyConstants._server, null);
