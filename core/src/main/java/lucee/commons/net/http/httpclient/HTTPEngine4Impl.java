...
public class HTTPEngine4Impl {
	 ...
	 * @throws IOException
	 */
	public static HTTPResponse post(URL url, String username, String password, long timeout, boolean redirect, String charset, String useragent,
	 ...
	 * @throws IOException
	 */
	public static HTTPResponse head(URL url, String username, String password, long timeout, boolean redirect, String charset, String useragent,
			ProxyData proxy, lucee.commons.net.http.Header[] headers) throws IOException {
	...
	private static Header toHeader(lucee.commons.net.http.Header header) {
		...
		if(header instanceof HeaderWrap)
			return ((HeaderWrap)header).header;
	...
	public static HttpClientBuilder getHttpClientBuilder() {
		...
		/*
		 * if(connMan==null) { connMan = new PoolingHttpClientConnectionManager(); // Increase max total connection to 200 connMan.setMaxTotal(200); // Increase
		 * default max connection per route to 20 connMan.setDefaultMaxPerRoute(20); // Increase max connections for localhost:80 to 50 //HttpHost localhost =
		 * new HttpHost("locahost", 80); //cm.setMaxPerRoute(new HttpRoute(localhost), 50); } builder.setConnectionManager(connMan);
		 */
		return builder;
	}
	...
	private static HTTPResponse _invoke(URL url, HttpUriRequest request, String username, String password, long timeout, boolean redirect, String charset,
			String useragent, ProxyData proxy, lucee.commons.net.http.Header[] headers, Map<String, String> formfields) throws IOException {
	...
	public static void setBody(HttpEntityEnclosingRequest req, Object body, String mimetype, String charset) throws IOException {
		if(body != null)
			req.setEntity(toHttpEntity(body, mimetype, charset));
	}
