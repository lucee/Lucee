...
public abstract class AbstractCaptcha {
	...
	private BufferedImage generate(String text,int width, int height, String[] fonts, boolean useAntiAlias
			, Color fontColor,int fontSize
			, int minOvals, int maxOvals, int minBGLines, int maxBGLines, int minFGLines, int maxFGLines, int startColor, int shear) throws CaptchaException {
	...
	private void drawRandomLine(Graphics2D graphics, Dimension dimension, Color lineColorType) {
		...
		graphics.drawLine((int)point1.getX(), 
				(int)point1.getY(), 
				(int)point2.getX(), 
				(int)point2.getY());
	}
