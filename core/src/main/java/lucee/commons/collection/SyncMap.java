...
public class SyncMap<K, V> implements MapPro<K, V>, Serializable {
	...
	public Set<K> keySet() {
		synchronized (mutex) {
			if (keySet == null) keySet = new SyncSet<K>(m.keySet(), mutex);
			return keySet;
		}
	}
	...
	public Set<Map.Entry<K, V>> entrySet() {
		synchronized (mutex) {
			if (entrySet == null) entrySet = new SyncSet<Map.Entry<K, V>>(m.entrySet(), mutex);
			return entrySet;
		}
	}
	...
	public Collection<V> values() {
		synchronized (mutex) {
			if (values == null) values = new SyncCollection<V>(m.values(), mutex);
			return values;
		}
	}
