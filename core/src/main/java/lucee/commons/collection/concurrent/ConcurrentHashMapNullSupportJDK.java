 ...
 *
 * <p><b>Class name history:</b> the {@code ConcurrentHashMapNullSupport} name in the same
 * package refers to the original pre-7.1 segmented implementation, restored under its
 * original name so pre-7.1 serialised sessions ({@code objectSave}, application/session
 * persistence, clustering) deserialise transparently. This class — the JDK-backed wrapper
 * introduced in LDEV-5098 — was given the {@code …JDK} suffix as part of LDEV-6288 to
 * preserve backwards compatibility on the wire. {@link MapFactory} routes the default
 * path here; the legacy path goes to {@code ConcurrentHashMapNullSupport}.
 */
public final class ConcurrentHashMapNullSupportJDK<K, V> implements Map<K, V>, Serializable {
	...
	public static <K, V> ConcurrentHashMapNullSupportJDK<K, V> withDelegate(Map<K, Object> delegate) {
		return new ConcurrentHashMapNullSupportJDK<>(delegate, true);
	}

	private ConcurrentHashMapNullSupportJDK(Map<K, Object> delegate, boolean unused) {
	 ...
	 */
	public ConcurrentHashMapNullSupportJDK(Map<K, Object> delegate) {
		this.delegate = delegate;
	}
	 ...
	 */
	public ConcurrentHashMapNullSupportJDK() {
	 ...
	 */
	public ConcurrentHashMapNullSupportJDK(int initialCapacity) {
	 ...
	 */
	public ConcurrentHashMapNullSupportJDK(int initialCapacity, float loadFactor) {
	 ...
	 * @throws IllegalArgumentException if the initial capacity is negative or the load factor is nonpositive
	 */
	public ConcurrentHashMapNullSupportJDK(int initialCapacity, float loadFactor, int concurrencyLevel) {
