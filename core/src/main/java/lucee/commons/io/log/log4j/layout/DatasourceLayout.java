	...
	public String format(LoggingEvent event) {
    	...
    	if(c!=null) {
	    	if(c instanceof ConfigWeb) 
	    		id=((ConfigWeb)c).getLabel();
	    	
	    	else 
	    		id=c.getIdentification().getId();
    	}
        
    	/*if(StringUtil.isEmpty(tableName)) {
    		Enumeration e = event.getLogger().getAllAppenders();
    		Object o;
    		while(e.hasMoreElements()) {
    			o = e.nextElement();
    			if(o instanceof DatasourceAppender) {
    				tableName=((DatasourceAppender)o).getTableName();
    				break;
    			}
    		}
    	}*/
		...
		+SQLCaster.toString(new SQLItemImpl(id,Types.VARCHAR))+","
		+sqlName+","
		+SQLCaster.toString(new SQLItemImpl(event.getLevel().toString(),Types.VARCHAR))+","
		+SQLCaster.toString(new SQLItemImpl(threadId,Types.VARCHAR))+","
		+new DateTimeImpl(event.getTimeStamp(), false)+","
		+SQLCaster.toString(new SQLItemImpl(application,Types.VARCHAR))+","
		+SQLCaster.toString(new SQLItemImpl(msg,Types.VARCHAR))+","
		+SQLCaster.toString(new SQLItemImpl(exception,Types.VARCHAR))+","
		+sqlCustom
