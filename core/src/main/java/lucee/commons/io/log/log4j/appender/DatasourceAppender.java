...
public class DatasourceAppender extends JDBCAppender implements Appender {
	...
	public DatasourceAppender(Config config, Layout layout, String datasource, String username, String password,
			String tableName, String custom) throws PageException {
	...
	private SQL createSQL(DatasourceConnection dc) {
		...
		if (DataSourceUtil.isMSSQL(dc))
			sb.append("dbo.");
		sb.append(tableName).append(" ( ").append("id varchar(32) NOT NULL, ").append("name varchar(128) NOT NULL, ")
				.append("severity varchar(16) NOT NULL, ").append("threadid varchar(64) NOT NULL, ")
				.append("time datetime NOT NULL, ").append("application varchar(64) NOT NULL, ")
				.append("message varchar(512) NOT NULL, ").append("exception varchar(2048) NOT NULL, ")
				.append("custom varchar(2048) NOT NULL ").append(");");
	...
	private void logConsole(LoggingEvent event) {
		...
		if (ti != null && (t = ti.getThrowable()) != null) {
			...
			getConsoleLogger().log(event.getLevel(), event.getMessage());
	}

	private Logger getConsoleLogger() {
		ConfigImpl config = (ConfigImpl) ThreadLocalPageContext.getConfig();
		if (logger == null) {
			LogAdapter la = (LogAdapter) config.getLog("console_datasource_appender", true); // TODO use log level from
																								// this logger...
			logger = la.getLogger();
		}
		return logger;
