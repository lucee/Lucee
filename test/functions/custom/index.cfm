...
<cfif FORM.Scene EQ 1>
	<cfset result = serializeJSON( data = data, queryFormat=false, useSecureJSONPrefix=true, useCustomSerializer=true)>
<cfelse>
	<cfset jsonObject = serializeJSON(data)>
	<Cfset result = deserializeJSON(json=jsonObject, strictmapping = true , usecustomserializer = true)>
</cfif>
