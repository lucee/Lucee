	...
	function run( testResults , testBox ) {
		describe( "test case for LDEV-2528", function() {
			it(title = "cfdocument with mixed-orientation default orientation landscape", body = function( currentSpec ) {
				document pagetype = "letter" orientation = "landscape" filename = "test-orientation.pdf" overwrite = "true" name = "local.output" {
					documentsection { echo("I am from landscape"); }
				}
				cfpdf(action = "read", source = "test-orientation.pdf", name = "local.out");
			...
			it(title = "cfdocument with mixed-orientation default orientation portrait", body = function( currentSpec ) {
				document pagetype = "letter" orientation = "portrait" filename = "test-orientation.pdf" overwrite = "true" name = "local.output" {
