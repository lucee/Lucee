	...
	function run( testResults , testBox ) {
		describe( "Test suite for LDEV-237", function() {
			it("Checking QOQ with Union of two queries", function( currentSpec ){
				var errorMsg = "";
				try {
					var MathsStuds = queryNew( "id", "integer", [{id=1},{id=2},{id=3}]);
					var ScienceStuds = queryNew( "id", "integer", [{id=3},{id=4},{id=5}]);

					var combinedStud = new Query(
						dbtype = "query",
					...
					var checkingExistenceOfintersectedRecord = new Query(
						dbtype = "query",
