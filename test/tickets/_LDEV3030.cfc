component extends = "org.lucee.cfml.test.LuceeTestCase" {
	...
	function run( testResults , testBox ) {
		describe( "Test suite for LDEV-3030", function() {
			...
			it(title = "Checking metadata with cf_sql_xml datatype in MSSQL", body = function( currentSpec ) {
			...
			it(title = "Checking null value with cf_sql_xml in MSSQL", body = function( currentSpec ) {
				result = _InternalRequest(
					template : "#uri#/LDEV3030.cfm",
			...
			it(title = "Checking metadata with cf_sql_xml datatype in jTDS MSSQL", body = function( currentSpec ) {
				result = _InternalRequest(
					template : "#uri#/LDEV3030.cfm",
			...
			it(title = "Checking null value with cf_sql_xml in jTDS MSSQL", body = function( currentSpec ) {
				result = _InternalRequest(
					template : "#uri#/LDEV3030.cfm",
