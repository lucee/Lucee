component extends="org.lucee.cfml.test.LuceeTestCase"{
	...
	function run( testResults , testBox ) {
		describe( title="Test suite for LDEV-215", body=function() {
			it( title='Checking MYSQL, INDEX for the client storage table',skip=isMySqlNotSupported(),body=function( currentSpec ) {
			...
			it( title='Checking MsSQL, INDEX for the client storage table',skip=isMsSqlNotSupported(),body=function( currentSpec ) {
				var uri = createURI("LDEV0215");
				var result = _InternalRequest(
