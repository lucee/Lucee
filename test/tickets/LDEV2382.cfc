component extends = "org.lucee.cfml.test.LuceeTestCase" labels="qoq" {
	...
	function run( testResults, testBox ){
		describe( "Test case for LDEV-2382",function() {
			it( title = "Query of Query -With upper() method", skip=true, body = function( currentSpec ){
			...
			it( title = "Query of Query - with lower() method", skip=true, body = function( currentSpec ){
				local.result = _InternalRequest(
					template : "#uri#\test.cfm",
			...
			it( title = "Query of Query - without used lower() and upper() method", skip=true, body = function( currentSpec ){
				local.result = _InternalRequest(
					template : "#uri#\test.cfm",
					form : {type = '',scene = '1'}
				)
