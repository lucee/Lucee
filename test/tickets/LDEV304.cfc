component extends="org.lucee.cfml.test.LuceeTestCase"{
	...
	function run( testResults , testBox ) {
		describe( "Test suite for LDEV-304", function() {
			it( title='Checking Query object does not return query result in cfquery', body=function( currentSpec ) {
				local.result = _InternalRequest(
					template:"#variables.uri#/test1.cfm");
				expect(result.filecontent.trim()).toBe("test");
			});
			it( title='Checking Query object does not return query result in query()', body=function( currentSpec ) {
				local.result = _InternalRequest(
